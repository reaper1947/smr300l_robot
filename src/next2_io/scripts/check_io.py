...
class DeviceNode(Node):
    def __init__(self):
        ...
        # Publishers
        # self.enble_pub = self.create_publisher(Bool, 'check_enable', 10)
        ...
        self.data_timer = self.create_timer(1.0, self.data_callback)
        self.check_timer = self.create_timer(1.0, self.check_command_rqt)

        # Serial initialization
        try:
            self.ser = serial.Serial(port='/dev/ttyACM0', baudrate=9600, parity='N', stopbits=1, bytesize=8, timeout=1.0)
    ...
    def sent_digital(self):
        ...
        if digital_byte & (0 << 0):  # Emergency (bit 0)
            self.status = 0x00
            self.button_status = False
            ...
            self.button_status = False
            self.msg_robot.data = False
        elif (digital_byte & (1 << 0)) and ((digital_byte & (1 << 1)) or (digital_byte & (1 << 2))):  # Master ON
            self.status = 0x03
