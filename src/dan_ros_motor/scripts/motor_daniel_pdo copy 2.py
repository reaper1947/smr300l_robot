...
class Ros2MotorNode(Node):
    ...
    def __init__(self):
        ...
        # CSV Log
        self.filepath = '/home/next/next_log/device/base_control/log.csv'
    ...
    def sdo_read_u16(self, motor_id, index, sub, timeout=0.2):
        req = can.Message(arbitration_id=0x600 | int(motor_id),data=[0x40, index & 0xFF, (index >> 8) & 0xFF, sub, 0, 0, 0, 0],is_extended_id=False)
        self.bus.send(req)
        resp = self.read_id([0x580 | int(motor_id)])
        if not resp or resp.data[0] != 0x4B:
            return None
        return resp.data[4] | (resp.data[5] << 8)

    def sdo_write_u16(self, motor_id, index, sub, value):
        value &= 0xFFFF
        msg = can.Message(
            arbitration_id=0x600 | int(motor_id),
            data=[0x2B, index & 0xFF, (index >> 8) & 0xFF, sub,
    ...
    def pulse_din4_polarity(self, motor_id, hold_ms=50):
        IDX, SUB = 0x2010, 0x01          # Din_Polarity
        DIN4_BIT = 1 << 3                # bit3

        cur = self.sdo_read_u16(motor_id, IDX, SUB)
        if cur is None:
            cur = 0

        self.sdo_write_u16(motor_id, IDX, SUB, cur | DIN4_BIT)   # set DIN4 bit
        time.sleep(hold_ms / 1000.0)
        self.sdo_write_u16(motor_id, IDX, SUB, cur)              # restore exact original
    ...
    def motor_transition(self, motor_id, transition):
        ...
        # DIN1 Unmap
            ...
            #self.bus.send(can.Message(arbitration_id=0x600 | motor_id,data=[0x2b, 0x40, 0x60, 0x00, 0x80, 0x00, 0x00, 0x00],is_extended_id=False))
            self.bus.send(can.Message(arbitration_id=0x600 | motor_id,data=[0x2B, 0x10, 0x20, 0x06, 0x02, 0x00, 0x00, 0x00],is_extended_id=False))
            self.pulse_din4_polarity(motor_id, hold_ms=50)
        # Enable Heartbeat
    ...
    def reset_fault(self, motor_id):
        ...
        msg_reset_high = can.Message(arbitration_id=0x600 | motor_id,
                                   ...
                                   is_extended_id=False)
        self.bus.send(msg_reset_high)

        time.sleep(0.1)

        msg_reset_low = can.Message(arbitration_id=0x600 | motor_id,
...
# --------- Check Motor Alarm (NEW VERSION) -----------
    ...
    def motor_alarm_check(self):
        ...
        try:
            if self.control_msg_r and self.control_msg_l:

                status_byte_r = self.control_msg_r.data[4]
                status_byte_l = self.control_msg_l.data[4]

                is_fault_r = (status_byte_r & 0x08) != 0
                ...
                if is_fault_r or is_fault_l:
                    ...
                    if is_fault_l:
                        self.reset_fault(self.motor_id_left)
