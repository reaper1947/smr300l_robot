...
class Ros2MotorNode(Node):
    ...
    def send_start_msg(self):
        ...
        check_alarm_msg = can.Message(arbitration_id=0x601, data=[0x23, 0xff, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00], is_extended_id=False)
        self.bus.send(check_alarm_msg)
        self.alarm_1 = self.bus.recv(timeout=0.1)

        # Send check alarm message for motor 2
        check_alarm_msg = can.Message(arbitration_id=0x602, data=[0x23, 0xff, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00], is_extended_id=False)
        self.bus.send(check_alarm_msg)
        ...
        if self.alarm_1.arbitration_id == 0x581 and self.alarm_2.arbitration_id == 0x582:
            ...
            if command_byte_1 == 0x60 and command_byte_2 == 0x60:
                ...
                if command_byte_1 == 0x80:
                    print(f"Motor 1 : Abort")
                else:
                    print(f"Motor 1 : Unknown command")
                if command_byte_2 == 0x80:
                    print(f"Motor 2 : Abort")
                else:
                    print(f"Motor 2 : Unknown command")
                self.disable_motor()
    ...
    def check_motor_enable(self):
        ...
        check_status_msg = can.Message(arbitration_id=0x601, data=[0x40, 0x41, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00], is_extended_id=False)
        self.bus.send(check_status_msg)
        self.status_1 = self.bus.recv(timeout=0.1)
        
        # Send check status message for motor 2
        check_status_msg = can.Message(arbitration_id=0x602, data=[0x40, 0x41, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00], is_extended_id=False)
        self.bus.send(check_status_msg)
    ...
    def spin_motor(self):
        ...
        left_motor_msg = can.Message(arbitration_id=0x601, data=data_frame_l, is_extended_id=False)
        right_motor_msg = can.Message(arbitration_id=0x602, data=data_frame_r, is_extended_id=False)
