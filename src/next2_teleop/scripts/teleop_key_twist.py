...
def main(args=None):
    ...
    use_stamped = '--stamped' in sys.argv
    node = TeleopNode(use_stamped)
        ...
        while True:
            ...
            if key == 'w':
                node.target_linear_vel = constrain(
                    node.target_linear_vel + LIN_VEL_STEP_SIZE, -MAX_LIN_VEL, MAX_LIN_VEL)
            elif key == 'x':
                node.target_linear_vel = constrain(
                    node.target_linear_vel - LIN_VEL_STEP_SIZE, -MAX_LIN_VEL, MAX_LIN_VEL)
            elif key == 'a':
                node.target_angular_vel = constrain(
                    node.target_angular_vel + ANG_VEL_STEP_SIZE, -MAX_ANG_VEL, MAX_ANG_VEL)
            elif key == 'd':
                node.target_angular_vel = constrain(
                    node.target_angular_vel - ANG_VEL_STEP_SIZE, -MAX_ANG_VEL, MAX_ANG_VEL)
            elif key in (' ', 's'):
