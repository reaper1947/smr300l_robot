...
class MotorController(Node):
    ...
    def standby_mode(self):
        if self.current_robotmode.robot_mode != 2 :
            self.logger.error(f"Current robot mode is {self.current_robotmode.robot_mode}, NOT 2")
            self.KillNode()
        else:
            # self.logger.info("Robot mode 2")
            self.current_state = 'enable_check'
    def ControlLoop_State(self):
        if self._isAllowed_Master_ON:
            if self.current_robotmode.robot_mode == 2:
                self.NMT_service(0x00, "Switch to the <Operational> state")
        elif not self._isAllowed_Master_ON:
            if self.current_robotmode.robot_mode == 5:
    ...
    def cmd_vel_callback(self, msg):
        try:
            if self._isAllowed_Master_ON == True:
                linear_velocity = msg.linear.x
                angular_velocity = msg.angular.z
            elif self._isAllowed_Master_ON == False:
                linear_velocity = 0
                angular_velocity = 0
    ...
    def DiagnosticsStatusWordnPublish(self):
        ...
        while not (received_181 and received_182):
            ...
            self.get_logger().info(f"0x181 0x181", throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_ReadytoSwitchON_181:  {self.STW_181['ReadytoSwitchON']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_SwitchedON_181:       {self.STW_181['SwitchedON']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_OperationEnabled_181: {self.STW_181['OperationEnabled']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_Fault_181:            {self.STW_181['Fault']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_VoltageEnabled_181:   {self.STW_181['VoltageEnabled']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_QuickStop_181:        {self.STW_181['QuickStop']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_SwitchONDisabled_181: {self.STW_181['SwitchONDisabled']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_Warning_181:          {self.STW_181['Warning']}",
                throttle_duration_sec=2)

            self.get_logger().info(f"0x182 0x182", throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_ReadytoSwitchON_182:  {self.STW_182['ReadytoSwitchON']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_SwitchedON_182:       {self.STW_182['SwitchedON']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_OperationEnabled_182: {self.STW_182['OperationEnabled']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_Fault_182:            {self.STW_182['Fault']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_VoltageEnabled_182:   {self.STW_182['VoltageEnabled']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_QuickStop_182:        {self.STW_182['QuickStop']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_SwitchONDisabled_182: {self.STW_182['SwitchONDisabled']}",
                throttle_duration_sec=2)
            self.get_logger().info(
                f"STW_Warning_182:          {self.STW_182['Warning']}",
                throttle_duration_sec=2)
    ...
    def SDO_ModeofOperation(self, MotorID, mode):
        ...
        if mode == "Profile Velocity Mode":
            ...
            success = True
        elif mode == "Profile Speed Mode":
            data_frame = [0x2f, 0x20, 0x20, 0x0e, 0x03, 0x00, 0x00, 0x00]
            success = True
        elif mode == "Profile Raw Speed Mode":
            data_frame = [0x2f, 0x20, 0x20, 0x0e, 0xfd, 0xff, 0xff, 0xff
]
            success = True
        else:
            success = False
    ...
    def QuickStop_dualMotor(self):
        ...
        if self._isMotor_1_OperationEn or self._isMotor_2_OperationEn:
            if self.device_name == "kinco":
                ...
                # self._isMotor_1_OperationEn = False
                # self._isMotor_2_OperationEn = False
                self.SDO_MotorSpin(self.motor_1_id, 0)
                # self.SDO_SendReqMotorActualSpeed(self.dual_motor_id)
                self.SDO_MotorSpin(self.motor_2_id, 0)
            else:
